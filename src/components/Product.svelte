...
<SectionWrapper id="product">
	...
	>
		<div class="pag-2 flex flex-col">
			<p class="text-center text-base opacity-60 sm:text-lg md:text-xl">
				Start your lifetime
				<span class="text-indigo-400">gym streak</span> today
			</p>
			<h3
		...
		{#each productFeatures as productFeature, index}
			<ProductCard {productFeature} {index}>
				{#if index === 0}
					Experience a beginner exercise <span class="font-medium text-indigo-400">master</span> class
				{:else if index === 1}
					Lay the foundations of your <span class="font-medium text-indigo-400">routine</span> and consistency
				{:else}
					Workouts specific to your <span class="font-medium text-indigo-400">environment</span> and
					<span class="font-medium text-indigo-400">expertise</span>
				{/if}
			</ProductCard>
		{/each}
	</div>
